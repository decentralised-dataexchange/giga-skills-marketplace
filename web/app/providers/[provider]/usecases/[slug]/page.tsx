...
import { ArrowLeft, ArrowRight, ArrowUpRight, Check, ListChecks } from "lucide-react";
...
import { providerPath, skillEntryPath, usecasePath, usecaseReviewPath } from "@/lib/routes";
...
export default function UsecasePage() {
  ...
  // The provider segment asserts ownership; a wrong one moves to the canonical URL.
  const ownerSlug: string | null = detail?.org?.slug ?? null;
  useEffect(() => {
    if (ownerSlug && ownerSlug !== provider) router.replace(usecasePath(ownerSlug, slug));
    ...
    <main className="mx-auto w-full max-w-[1536px] px-5 sm:px-6 lg:px-8 pb-20 pt-8">
      ...
      <div className="mt-3 border-b border-border pb-5">
        ...
        <div className="mt-4 flex flex-wrap items-center gap-x-5 gap-y-2 text-sm text-muted-foreground">
          ...
          <Link
            href={usecaseReviewPath(handle, skill.slug)}
