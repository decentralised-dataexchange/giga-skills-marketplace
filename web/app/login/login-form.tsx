...
import type { DemoAccountView } from "@/lib/demo-accounts";
...
/**
 * The sign-in card. Both props come from the server page, which reads the
 * marketplace settings: `demoAccounts` is the demo account list while demo
 * mode is on (else empty, and the "Use a demo account" link and drawer do
 * not exist); `selfServiceRegistration` false removes "Create account".
 */
    ...
    <main className="flex flex-1 justify-center bg-white px-4 pt-[14vh] pb-12 max-[520px]:pt-[6vh]">
      <div className="h-fit w-[min(468px,100%)]">
        ...
        >
          ...
          {/* The alternate action (create an account, or back to sign-in)
            fills black on hover, like the reference wallet sign-in button.
            Absent when a super admin has closed self-service registration. */}
          {selfServiceRegistration && (
            <>
        ...
        {/* Below the card: a low-stakes way in for visitors who just want a
            look. Demo deployments only; production mode passes no accounts. */}
        {hasDemoAccounts && (
      ...
      {hasDemoAccounts && demoOpen && (
        ...
        >
          ...
          <ul className="divide-y divide-border rounded-lg border border-[#e0e0e0] bg-white">
            {demoAccounts.map(({ email, password, label }) => (
              <li key={email}>
                <button
                  type="button"
                  onClick={() => {
