...
export default function ReviewQueuePage() {
  ...
  async function decide(decision: string) {
    ...
    await api(`/api/review/versions/${review.version.id}/decision`, { method: "POST", json: { decision, notes, official } });
    setReview(null);
    load();
  }
  ...
  return (
    <DashboardMain title="Review queue" subtitle="Claim submissions, inspect the bundle and checks, and record a decision." denied={denied}>
      ...
      {review && (
        <Card className="gap-4 p-6">
              ...
              <div className="mb-2 flex flex-wrap gap-1.5">
                {review.version.files.map((f: any) => (
                  <button key={f.path} onClick={() => setActiveFile(f.path)}
                    className={cn("rounded-md px-2.5 py-1.5 font-mono text-xs",
          ...
          <div className="space-y-2">
            <label htmlFor="review-notes" className="text-sm font-medium">Reviewer notes (shown to the provider, kept as audit evidence)</label>
            <MarkdownEditor id="review-notes" value={notes} onChange={setNotes}
              placeholder="e.g. OpenAPI surface matches the declared protocols; rulebook covers revocation; approved." />
            <label className="flex items-center gap-2 text-sm">
              <input type="checkbox" className="size-4 accent-brand" checked={official} onChange={(e) => setOfficial(e.target.checked)} />
      ...
      <Card className="gap-3 p-6">
        <h2 className="font-medium">Awaiting review <span className="tabular-nums text-muted-foreground">({queue.length})</span></h2>
        {queue.length ? (
          <>
            <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-sm">
              <thead>
                <tr className="text-left text-xs uppercase tracking-wide text-muted-foreground">
                  <th className="py-2">Name</th><th>Type</th><th>Provider</th><th>Status</th><th>Reviewer</th><th>Submitted</th><th />
                </tr>
              </thead>
              <tbody>
                  ...
                  <tr key={v.id} className="border-t border-border">
                    ...
                    <td>
                      <span className={cn("rounded-full px-2 py-0.5 text-xs font-semibold",
                        v.type === "usecase" ? "bg-brand/10 text-brand" : "bg-secondary text-ink/70")}>
