...
export default function OrganisationsPage() {
  ...
  return (
    <DashboardMain title="Organisations" subtitle="Verify provider organisations before they can publish." denied={denied}>
      {message && <p className="text-sm font-semibold text-amber-600">{message}</p>}
      <Card className="gap-3 p-6">
        <h2 className="font-medium">Awaiting verification <span className="tabular-nums text-muted-foreground">({pending.length})</span></h2>
        {pending.length ? (
          pending.map((o) => (
            <div key={o.id} className="border-t border-border pt-3">
              ...
              <p className="text-sm text-muted-foreground">
                {o.description}{" "}
                {o.website && <a className="font-semibold text-brand" href={o.website} target="_blank" rel="noopener">{o.website}</a>}
              </p>
              <p className="text-xs text-muted-foreground">Contact: {o.contact} · Owner: {o.owner.name} ({o.owner.email})</p>
              <div className="mt-2 flex gap-2">
                <Button size="sm" onClick={act(() => api(`/api/admin/orgs/${o.id}/decision`, { method: "POST", json: { decision: "approve" } }))}>
                  Approve
                </Button>
                <Button size="sm" variant="destructive"
                  onClick={act(() => api(`/api/admin/orgs/${o.id}/decision`, { method: "POST", json: { decision: "reject", notes: prompt("Reason for rejection:") ?? "" } }))}>
                  Reject
                </Button>
              </div>
            </div>
        ...
        <div className="overflow-x-auto"><table className="w-full min-w-[720px] text-sm">
          <thead>
            <tr className="text-left text-xs uppercase tracking-wide text-muted-foreground">
              <th className="py-2">Name</th><th>Status</th><th>Owner</th><th>Created</th>
            </tr>
          </thead>
          <tbody>
            {orgs.map((o) => (
