...
export default function ApplicationsModerationPage() {
  ...
  return (
    <DashboardMain
      title="Applications"
      subtitle="Moderate developer showcase submissions."
      denied={denied}
    >
      ...
      <Card className="gap-3 p-6">
        <h2 className="font-medium">
          ...
          <span className="tabular-nums text-muted-foreground">({apps.length})</span>
        </h2>
        {apps.length ? (
          <>
                  ...
                  {pageSlice(apps, page, PAGE).map((a) => (
                    <tr key={a.id} className="border-t border-border">
                      ...
                      <td className="text-right">
                        <Button
                          size="sm"
                          variant={a.status === "published" ? "destructive" : "secondary"}
                          onClick={act(() =>
                            api(`/api/applications/${a.id}/status`, {
                              method: "POST",
                              json: { status: a.status === "published" ? "delisted" : "published" },
                            }),
                          )}
                        >
