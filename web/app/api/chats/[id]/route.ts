...
export const PUT = route<{ id: string }>(async ({ user, params, body }) => {
  const b = await body<ChatBody>();
  const [current] = await sql`SELECT * FROM chats WHERE id = ${params.id} AND user_id = ${user!.id}`;
  check(current, 404, "Chat not found");
  const messages = cleanMessages(b.messages);
  const appHtml = "appHtml" in b
...
export const DELETE = route<{ id: string }>(async ({ user, params }) => {
  const rows = await sql`DELETE FROM chats WHERE id = ${params.id} AND user_id = ${user!.id} RETURNING id`;
