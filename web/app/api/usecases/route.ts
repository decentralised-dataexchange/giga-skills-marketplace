...
export const POST = route(async ({ user, body }) => {
  ...
  if (!skill) [skill] = await sql`INSERT INTO skills (slug, org_id, type) VALUES (${slug}, ${org.id}, 'usecase') RETURNING *`;
  ...
  const [version] = await sql`
    ...
    RETURNING *`;
  await logEvent(passed ? "usecase.submitted" : "usecase.checks_failed", user!.id,
    { skillId: skill.id, versionId: version.id }, { slug });
