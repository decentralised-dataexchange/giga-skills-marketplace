...
export const POST = route(async ({ user, body }) => {
  const { orgId, files } = await body<{ orgId: number; files: BundleFile[] }>();
  check(Array.isArray(files) && files.length > 0, 400, "files[] is required: [{ path, content }]");
  for (const f of files) check(!f.path.includes("..") && !f.path.startsWith("/"), 400, `Illegal path: ${f.path}`);
  check(files.reduce((n, f) => n + f.content.length, 0) <= MAX_BUNDLE_BYTES, 400, "Bundle exceeds the 2 MB limit");

  const [org] = await sql`SELECT * FROM orgs WHERE id = ${orgId} AND owner_id = ${user!.id}`;
  check(org, 404, "Organisation not found");
  check(org.status === "approved", 403, "Your organisation must be approved before publishing skills");
  ...
  const [version] = await sql`
    ...
    RETURNING *`;
  await logEvent(passed ? "skill.submitted" : "skill.checks_failed", user!.id,
    { skillId: skill.id, versionId: version.id }, { slug, version: version.version });
