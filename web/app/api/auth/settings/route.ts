...
export const GET = route(async ({ user }) => ({ settings: settingsView(user!.settings) }), { auth: true });

export const PUT = route(async ({ user, body }) => {
  const { openrouterKey, model, clearKey } = await body<{ openrouterKey?: string; model?: string; clearKey?: boolean }>();
  ...
  await logEvent("user.settings_updated", user!.id, { userId: user!.id },
