...
export const POST = route(async ({ body }) => {
  ...
  check(user && verifyPassword(password ?? "", user.password_hash), 401, "Invalid email or password");
  check(user.status === "active", 403, "This account is suspended - contact the marketplace operator");
