...
export const GET = route(async ({ req }) => {
  ...
  const like = `%${q}%`;
  const qCond = q ? sql`AND (lower(a.title) LIKE ${like} OR lower(coalesce(a.description,'')) LIKE ${like})` : sql``;
...
export const POST = route(async ({ user, body }) => {
  ...
  const [app] = await sql`
    ...
    RETURNING *`;
  await logEvent("application.submitted", user!.id, { applicationId: app.id }, { title: app.title });
