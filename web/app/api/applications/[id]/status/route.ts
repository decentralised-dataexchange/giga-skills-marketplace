...
export const POST = route<{ id: string }>(async ({ user, params, body }) => {
  ...
  check(status === "published" || status === "delisted", 400, "status must be published or delisted");
  ...
  await logEvent("application.moderated", user!.id, { applicationId: app.id }, { title: app.title, status });
