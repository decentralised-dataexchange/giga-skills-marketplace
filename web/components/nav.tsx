    ...
    <header className={cn("sticky top-0 z-20 border-b border-border bg-white/90 backdrop-blur-md", className)}>
      <div className="mx-auto flex h-16 max-w-[1536px] items-center gap-1 px-5 sm:px-6 lg:px-8">
        ...
        <div className="hidden items-center gap-1 md:flex">
          {user ? (
            <>
              <Button variant="ghost" size="sm" nativeButton={false} render={<Link href={primaryConsole(user)} />}>
                Dashboard
              </Button>
              <Link href="/settings" className="flex items-center gap-2 rounded-full py-1 pl-1 pr-3 text-sm font-medium text-ink transition-colors hover:bg-secondary">
      ...
      {open && (
        <nav aria-label="Primary" className="space-y-1 border-t border-border px-4 py-3 md:hidden">
          ...
          <div className="border-t border-border pt-2">
            {user ? (
              <>
                <Link href={primaryConsole(user)} onClick={() => setOpen(false)} className="block rounded-lg px-3 py-2 text-sm font-semibold text-ink/70 hover:bg-secondary hover:text-ink">
                  Dashboard
                </Link>
                <Link href="/settings" onClick={() => setOpen(false)} className="flex items-center gap-2 rounded-lg px-3 py-2 text-sm font-semibold text-ink/70 hover:bg-secondary hover:text-ink">
                  <UserAvatar name={user.name} avatar={user.avatar} size="sm" decorative /> {user.name}
                </Link>
                <button type="button" onClick={signOut} className="block w-full rounded-lg px-3 py-2 text-left text-sm font-semibold text-ink/70 hover:bg-secondary hover:text-ink">
                  Sign out
                </button>
              </>
            ) : (
              <Link href="/login" onClick={() => setOpen(false)} className="block rounded-lg px-3 py-2 text-sm font-semibold text-brand hover:bg-secondary">
                Sign in
              </Link>
            )}
          </div>
