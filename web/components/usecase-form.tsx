...
const emptyJourney = (): JourneyDraft => ({
  title: "",
  description: "",
  done: "",
  prompts: [emptyStep()],
...
export function UsecaseForm({
  ...
  function setStep(ji: number, si: number, patch: Partial<StepDraft>) {
    setJourneys((js) =>
      js.map((j, k) =>
        k === ji
          ? { ...j, prompts: j.prompts.map((p, m) => (m === si ? { ...p, ...patch } : p)) }
          : j,
      ),
    );
  }
    ...
    <form className="space-y-4" onSubmit={submit}>
      <div className="grid gap-4 sm:grid-cols-2">
        <div className="space-y-1.5">
          <label htmlFor="uc-name" className="text-sm font-medium text-ink">
            Name (slug)
          </label>
          <Input
            id="uc-name"
            placeholder="national-learner-registry"
            value={name}
      ...
      <div className="space-y-1.5">
        <label htmlFor="uc-title" className="text-sm font-medium text-ink">
          Title
        </label>
        <Input
          id="uc-title"
          placeholder="National Learner Registry & Digital Credentials"
          value={title}
      ...
      <div className="space-y-1.5">
        ...
        <MarkdownEditor
          ...
          placeholder="What working solution this use case delivers."
        />
      </div>
          ...
          <div key={i} className="flex gap-2">
            ...
            {prereqs.length > 1 && (
              <button
                ...
                onClick={() => setPrereqs(prereqs.filter((_, k) => k !== i))}
          ...
          <div key={ji} className="space-y-3 rounded-lg border border-border p-4">
            <div className="flex items-center justify-between">
              ...
              {journeys.length > 1 && (
                <button
                  type="button"
                  aria-label={`Remove journey ${ji + 1}`}
                  onClick={() => setJourneys((js) => js.filter((_, k) => k !== ji))}
            ...
            <Input
              ...
              value={j.title}
              onChange={(e) => setJourney(ji, { title: e.target.value })}
              required
            />
            <div className="space-y-1.5">
              ...
              <MarkdownEditor
                value={j.description}
                onChange={(v) => setJourney(ji, { description: v })}
            ...
            <div className="space-y-2 rounded-lg bg-muted/40 p-3">
                ...
                <div key={si} className="space-y-2 rounded-lg border border-border bg-white p-3">
                  <div className="flex items-center justify-between">
                    ...
                    {j.prompts.length > 1 && (
                      <button
                        type="button"
                        aria-label={`Remove prompt ${si + 1}`}
                        onClick={() =>
                          setJourney(ji, { prompts: j.prompts.filter((_, m) => m !== si) })
                        }
                        className="text-muted-foreground hover:text-destructive"
                      >
                  ...
                  <MarkdownEditor
                    value={p.prompt}
                    onChange={(v) => setStep(ji, si, { prompt: v })}
                    placeholder="The agent prompt for this step."
                    minHeightClass="min-h-20"
                  />
                  <MultiSelect
                    label="Skills"
                    ...
                    selected={p.skills}
                    onChange={(v) => setStep(ji, si, { skills: v })}
              ...
              <Button
                ...
                variant="secondary"
                size="sm"
                onClick={() => setJourney(ji, { prompts: [...j.prompts, emptyStep()] })}
              >
            ...
            <Input
              placeholder="Done when… (acceptance criterion)"
              value={j.done}
              onChange={(e) => setJourney(ji, { done: e.target.value })}
            />
          </div>
        ))}
        <Button
          ...
          variant="secondary"
          size="sm"
          onClick={() => setJourneys((js) => [...js, emptyJourney()])}
        >
