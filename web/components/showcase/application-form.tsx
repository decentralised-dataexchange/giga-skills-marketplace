...
import { patchExchange } from "@/lib/showcase/store";
import {
  readApplicationData,
  startApplicationRequest,
} from "@/app/showcase/civicworks/verify/actions";
import type { VerificationResult } from "@/lib/showcase/server/verification";
import type { Job } from "@/lib/showcase/jobs";
 ...
 * candidate submits when satisfied. The success screen is generic; the
 * submission itself is only an audit event in this browser's trail.
 */
export function ApplicationForm({ job }: { job: Job }) {
  ...
  const beginWalletFill = useCallback(async () => {
    ...
    setError(null);
    try {
      const request = await startApplicationRequest();
      setWalletRequest(request);
      patchExchange(request.exchangeId, {
        direction: "presentation",
        credentialType: "diploma-verify",
        status: "request_sent",
      });
      void audit({
        actorPseudonym: null,
  ...
  const onWalletEvent = useCallback(
    async (payload: Record<string, unknown>) => {
      ...
      patchExchange(exchangeId, { status: topic });
  ...
  useExchangeStatus(walletRequest?.exchangeId ?? null, "civicworks", "presentation", onWalletEvent);
    ...
    await audit({
      ...
      payload: {
        ...
        ...(evidence.kind === "wallet"
          ? { presentationExchangeId: evidence.result.exchangeId }
          : {}),
      },
    });
    setSubmitted(true);
          ...
          {evidence.kind === "pdf" ? (
            ...
            <div className="cw-attestation">
              <div className="cw-attestation-head">
                ...
                <img src="/showcase/portals/moe/logo.svg" alt="" />
                <div>
